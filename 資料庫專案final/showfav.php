...
if (isset($_SESSION['member'])) {
	...
	FROM favorites
	INNER JOIN drama
	ON favorites.dNo=drama.dNo WHERE favorites.mId='$mId' ";

    $result = mysqli_query($db_link, $sql);
    echo '<main id="main" style="text-align: center;">';
    echo "<p class='ct fb'>收藏夾</p>";
    while($row=mysqli_fetch_assoc($result)){       
		echo '<table class="table">';
		echo '<th>戲劇編號</th><th>戲劇名稱</th><th>圖片</th>';
			echo '<tr>';
			echo '<td>', $row['dNo'], '</td>';
			echo '<td><a href="detail.php?id='.$row["dNo"].'">', $row['dName'],'</a></td>';
			echo '<td>';
			echo '<img src = " '.'data:image/jpg;base64,'.base64_encode($row["img"]).'" class = "photo">';
			echo '</td>';
			echo '<td><a href="fav-del.php?id='.$row["dNo"].'">刪除</a></td>';
			echo '</tr>';
		
		echo '</table>';

    // echo  '<div class="drama" style="text-align: center;">';
